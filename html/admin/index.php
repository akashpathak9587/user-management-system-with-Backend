<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>User Login | Registration and Login System</title>
    <link rel="stylesheet" href="../../css/admin.css">
    <link rel="stylesheet" href="../../css/font-awesome.css">
</head>

<body>
    <div class="main-form">
        <div class="form-header">
            <div class="main-heading">
                <h1>Registration and Login System</h1>
            </div>
            <div class="sub-heading">
                <h1>Admin Login </h1>
            </div>
        </div>
        <div class="logForm">
            <form action="dashboard.html" method="post">
                <input id="email" type="email" name="email" placeholder="Username">
                <input id="pass" type="password" name="pass" placeholder="password"><a href="#">Forgot Password?</a>
                <input type="submit" value="Login">
            </form>
        </div>
        <div class="formFooter"><a href="../index.html" style="color: #11b8de">Back to Home</a></div>
    </div>
    <footer>
        <p>copyright &#169; 2022</p><a href="#">Akash Pathak</a>
    </footer>
</body>

</html>